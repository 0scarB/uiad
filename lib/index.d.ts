...
type ReactiveResult<
    T,
    PostProcessorErrKind extends string = ReactiveValuePostProcessorDefaultErrKind,
...
export type ReactiveValuePostProcessorSpec<
    ...
    TransformationErrKind extends string = ReactiveValuePostProcessorDefaultErrKind,
...
> =
    ...
    | [
        "transform", 
        string, 
        ReactiveValueTransformation<
            In,
            Out,
            TransformationErrKind 
        >
    ]
    | [
        "validate", 
        string, 
        ReactiveValueValidator<
            In,
            ValidatorErrKind 
        >
    ]
...
export interface ReactiveValue<
    In,
    Out = In,
> extends Reactive<Out> {
    get(): Out
    set(newValue: In): ReactiveResult<ReactiveValue<In, Out>>,
    update(transform: (oldValue: Out) => In): ReactiveResult<ReactiveValue<In, Out>>,
}
...
> = {
    ...
    onErr(callback: (errKind: ErrKind, errMsg: string) => unknown): Ok<OkValue, ErrKind>,
    mapOk<
        T extends 
            ((value: OkValue) => unknown)
            | unknown
        = (value: OkValue) => unknown
    >(
        new_: T,
        ...
        : T extends (value: OkValue) => Err<infer NewErrKind, infer NewOkValue>
        ? Err<NewErrKind, NewOkValue>
        : Ok<T, ErrKind>
    ),
    mapErr<
        T extends 
            ((errKind: ErrKind, msg: string) => (
                string
                | [string, string]
    ...
    >(
        ...
        T extends ((errKind: ErrKind, msg: string) => infer NewErrKind extends string)
        ? Ok<OkValue, NewErrKind>
        : T extends (errKind: ErrKind, msg: string) => [infer NewErrKind extends string, string]
        ? Ok<OkValue, NewErrKind>
        : T extends (errKind: ErrKind, msg: string) => Ok<any, string>
        ? Ok<OkValue, ErrKind>
        : T extends (errKind: ErrKind, msg: string) => Ok<any, infer NewErrKind>
        ? Ok<OkValue, NewErrKind>
        : T extends (errKind: ErrKind, msg: string) => Err<infer NewErrKind, any>
        ? Ok<OkValue, NewErrKind>
    ...
    logErr<ConsoleMeth extends keyof Console = "error">(
        ...args:
            [
                ConsoleMeth, 
                Parameters<Console[ConsoleMeth]>[0]
                | ((errKind: ErrKind, msg: string) => Parameters<Console[ConsoleMeth]>[0])
            ]
            | [
                Parameters<Console[ConsoleMeth]>[0]
                | ((errKind: ErrKind, msg: string) => Parameters<Console[ConsoleMeth]>[0])
...
> = {
    ...
    onErr(callback: (errKind: ErrKind, errMsg: string) => unknown): Err<ErrKind, OkValue>,
    logOk<ConsoleMeth extends keyof Console = "log">(
        ...args:
            [
                ConsoleMeth, 
    ...
    >(
        ...
        : T extends (errKind: ErrKind, msg: string) => [infer NewErrKind extends string, string]
        ? Err<NewErrKind, OkValue>
        : T extends (errKind: ErrKind, msg: string) => Ok<infer NewOkValue, string>
        ? Ok<NewOkValue, ErrKind>
        : T extends (errKind: ErrKind, msg: string) => Ok<infer NewOkValue, infer NewErrKind>
        ? Ok<NewOkValue, NewErrKind>
        : T extends (errKind: ErrKind, msg: string) => Err<infer NewErrKind, unknown>
        ? Err<NewErrKind, OkValue>
        : T extends (errKind: ErrKind, msg: string) => Err<infer NewErrKind, infer NewOkValue>
        ? Err<NewErrKind, NewOkValue>
        : T extends [infer NewErrKind extends string, string]
        ? Err<NewErrKind, OkValue>
        : T extends string
...
export type Result<
    OkValue,
    ErrKind extends string = string,
> = Ok<OkValue, ErrKind> | Err<ErrKind, OkValue>

export function ok<
    OkValue,
    ErrKind extends string = string,
>(value: OkValue): Ok<OkValue, ErrKind>
