...
class Err {
    ...
    ifErrThrow(jsError) {
        switch (typeof jsError) {
            ...
            case "function":
                return this.ifErrThrow(jsError(this.errKind, this.errMsg));
        }
...
export function createElement(spec) {
    ...
    throw new Error(`Cannot create a DOM node from "${JSON.stringify(spec)}"!`);
}
...
export function readOnlyReactiveValue(value, ...postProcessorsSpec) {
    return createReactiveValue(value, true, postProcessorsSpec);
}

export function readWriteReactiveValue(value, ...postProcessorsSpec) {
    return createReactiveValue(value, false, postProcessorsSpec);
}

function createReactiveValue(value, isReadOnly, postProcessorsSpec) {
    ...
    if (postProcessorsSpec.length > 0) {
        ...
        populatePostProcessorMap(postProcessors, postProcessorsSpec);
    }

    let reactiveValue;
    if (isReactiveValue) {
        reactiveValue = ReactiveValue(value.get(), isReadOnly, postProcessors);
    } else {
        reactiveValue = new ReactiveValue(value, isReadOnly, postProcessors);
    }
...
export function reactiveElement(value, render, options) {
    let element;
    if (
